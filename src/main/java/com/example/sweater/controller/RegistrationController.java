...
public class RegistrationController {
    ...
    public String addUser(
        ...
        if (isConfirmEmpty) {
            model.addAttribute("password2Error", "Password confirmation be empty");
        }
        if (user.getPassword() != null && differentPasswords) {
            model.addAttribute("passwordError", "Password are different");
        }

        if (isConfirmEmpty || differentPasswords || bindingResult.hasErrors() || (response != null && !response.isSuccess())) {
    ...
    public String activate(Model model, @PathVariable String code) {
        ...
        if (isActivated) {
            ...
            model.addAttribute("message", "User successfully activates");
        } else {
